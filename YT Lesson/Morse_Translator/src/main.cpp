 ...
 * ********************************************************* */
    ...
    // Not included yet , need to find extra symbols
    String ProSigns[] = {
        "...-.-",   // End Of Work
        "........", // Error
...
void loop(){
    ...
    if (Serial.peek() == 27) {                  // Check if the next character is the Escape key
        Serial.println("Exiting the Morse Code Translator.");
        while (true) {                          // Infinite loop to halt further execution
            delay(1000);                        // Delay to keep the system from overloading
        }
    }
