 ...
 *   modified by Charis Zachariadis
 * 
 *
 * Description:
...
void sevenSegWrite(byte digit, bool bDP = false, char switchValue='D') {
    ...
    if (switchValue=='A'){
        ...
        shiftOut(dataPin, clockPin, MSBFIRST, sevenSegAlpha[digit]+(sevenSegDP*bDP)); // add decimal point with OR and shifting using the sevenSegDP, writes using the sevenSegAlpha array
    } else if (switchValue=='B'){
        // binary (lights up individual segments)
        shiftOut(dataPin, clockPin, MSBFIRST, digit+(sevenSegDP*bDP)); // same with the other but having the digit as the value
    } else {
        // digits (from 0 to 9)
        shiftOut(dataPin, clockPin, MSBFIRST, sevenSegDigits[digit]+(sevenSegDP*bDP)); // similar to 'A' but using the sevenSegDigits array
    }
...
void loop() {
    ...
    // SECRET MESSAGE
    sevenSegWrite(0x60,false,'B'); // using 'B' to create custom patterns
    delay(500);
    sevenSegWrite(0x70,false,'B');  
    delay(500);
    sevenSegWrite(0x7B,false,'B');  
    delay(500);
    sevenSegWrite(0x79,false,'B');  
    delay(500);
    sevenSegWrite(0xF2,false,'B');  
    delay(500);
    sevenSegBlank();
    delay(500);
    sevenSegWrite(0xC9,false,'B');  
    delay(500);
    sevenSegWrite(0x7B,false,'B');  
    delay(500);
    sevenSegWrite(0x7D,false,'B');  
    delay(500);
    sevenSegBlank();
    delay(1000);
